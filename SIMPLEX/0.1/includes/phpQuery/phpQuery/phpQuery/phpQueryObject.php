...
//									: null;
    ...
    private function addStyle($array_rules){
        $style = $this->attr('style');
        $rules =array();
        if($style){
           $style = explode(";",$style) ;
           foreach($style as $rule){
               $key_value = explode(':',$rule);
               if(isset($key_value[0]) && isset($key_value[1])){
                    $rules[$key_value[0]] = $key_value[1];
               }
           }
        }
        if(is_array($array_rules)){
        ...
        if($rules){
            ...
            $this->attr('style',implode(';',$pieces ));
        }
    }
	/**
	 * Enter description here...
	...
	public function show(){
        $this->addStyle(array('display'=>'block'));
		// TODO
		return $this;
	}
	/**
