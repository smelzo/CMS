...
;(function(){ //linkedElement plugin
	linkedElement = function (e,options){
		options = $.extend({
			cls:'handy',
			hcls:'hover',
		...
		var le = $(options.template).css({position:'absolute',zIndex:maxZ()}).addClass(options.cls).appendTo('body');
		var positioning = function (){
			...
			if($.isFunction(options.positioning)){
				options.positioning(e,le,options);
			}
			var max = Math.max;
			var offset = $(e).offset();
			var w = max( $(e).outerWidth(), 16);
			var h = max( $(e).outerHeight(), 16);
			var we = max( $(le).outerWidth(), 16);
			var he = max( $(le).outerHeight(), 16);
			...
			if(options.verticalPosition=='top') {
				props.top = offset.top + options.verticalOffset - (he/2);
			}
			else if(options.verticalPosition=='bottom') {
				props.top = offset.top + options.verticalOffset + h - he - (he/2);
			}
			else if(options.verticalPosition=='center') {
				props.top = offset.top  + (h/2) - (he/2) ;
			}
			if(options.horizontalPosition=='left') {
				props.left = offset.left + options.horizontalOffset - (we/2);
			}
			else if(options.horizontalPosition=='right') {
				props.left = offset.left + options.horizontalOffset + w - we ;
			}
			else if(options.horizontalPosition=='center') {
				props.left = offset.left  + (w/2) - (we/2) ;
			}
			if(props.top>wh || props.left>ww) {
				le.hide();
				return ;
			}
			else {
				if(options.markerShow && $(options.markerShow).length==0) {
					le.hide();
					return ;
				}
				le.show();
		...
		le.on('mouseenter',function (){
			$(e).addClass(options.hcls);
		});
		le.on('mouseleave',function (){
			$(e).removeClass(options.hcls);
		});
	...
	$.fn.linkedElement = function(options) {
		return linkedElement(this.get(0),options);
	};
})();
var uniquid = function (prefix){
	prefix = prefix || 'tmp_';
	var d = new Date();
	return prefix + d.valueOf();
}
...
Path = {
	...
	parent : function (){
		var str = this.value,
			sep = this.separator,
			liof = str.lastIndexOf(sep);
		if(liof>=0) {
			this.set(str.substring(0,liof));
		}
		return this;
	},
	join:function (path){
		return this.set( this.get() + this.separator + path);
	}
}
/**
 * load_list = Array [{src:<path>,type:'css||script'}]
*/
Loader = function (load_list,options){
    options = $.extend({
        end : $.noop
		,base:''
        ,step: $.noop
    ...
    var _script = function (){
        $.getScript(current.src, function() {
                            _next()
                        }).error(function (){
                            console.log('error');
                            _next()
    ...
    var _load = function (){
         current = load_list[load_index];
         switch(current.type){
              case 'css': _css(); _next(); break;
			  case 'function':
				current.src();
				_next(); break;
              case 'script':_script();break;
         }
    }
    _load();
}
