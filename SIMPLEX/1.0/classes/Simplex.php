...
*/    static function eval_alt_template(&$buffer){
        ...
        $alt_template = TEMPLATES_PATH._."page-$current_page.php";
    ...
    static function display(){
        ...
        if(querystring('hooks')){
            file_put_contents(BASE_PATH._.'called_hooks.txt',implode("\n",$GLOBALS['HOOKS']->get_called_hooks()));
        }
    }
    
    static function save_page_html_fragment($pagename,$id_element,$lang,$html){
        $file = self::check_page_name($pagename);
        if(!$file) return  null;
        $doc = phpQuery::newDocumentFilePHP($file);
        $nd =  $doc->find("#{$id_element}[lang={$lang}]");
        if($nd->length) {
            $nd->html($html);
        }
    ...
    */
    static function get_pagename(&$type=0){
        ...
        if($default_pagename=='index') $default_pagename='home'; //index is renamed in home!
    ...
    static function evalued_include($filename = "", $params=array()){
        if(!is_file($filename)) {
            ...
            return APPLICATION_PATH . _ . "not_found_include.php";
        }
        return $filename;
    }
    ...
    static function template_include($name = "", $params=array()){
        $file = TEMPLATES_PATH._."$name.php";
        return self::evalued_include($file);
    }
    ...
    */
    static function context_script($script_path=null,$script_name=null,$script_prefix=null,$script_suffix=null){
        ...
        if(is_file(BASE_PATH ._.$script_path)){
            ?>
    ...
    */
    static function context_css($css_path=null,$css_name=null,$css_prefix=null,$css_suffix=null){
        ...
        $css_path.= "/$css_name";
        if(is_file(BASE_PATH ._.$css_path)){
            ?>
