...
var News = {
    ...
    ,prependListItem : function (item){
        ...
        var appendFx =  (listAppendType=='append')?'append':'prepend';
        newsList[appendFx](e);
    ...
    ,fillPanelList : function (data,html){
        ...
        $.each(data,function (i,item){
            var e = $(html);
            if (!showDate)  {
                e.find('[data-content="creation"]').parent().remove()
            }
            if (!showAbstract)  {
                e.find('[data-content="abstract"]').parent().remove()
            }
        ...
        var onSortList = function (){
            var series = [];
            $(newsList).find('>li[data-id]').each(function (i,el){
                var id = $(el).attr('data-id');
                series.push({id:id,position:(i+1)});
            });
            if(series.length) {
                $.serverPost('News.setPositions',{table:'news',series:$.encode(series)},function (){
                    self.showSaveMsg();
                })
            }
        }
    ...
    ,setupCategories : function (item){ // usabile in refresh
        ...
        if(!self.categoriesControl.length) return ;
    ...
    ,setFileDownload : function (url,li){
        ...
        var select_lang = li.find('select[name="lang"]');
        var lang = window.lang;
        if (select_lang.length) {
            var selected_lang = select_lang.val();
            if (!selected_lang)  {
                selected_lang=null;
            }
            lang=selected_lang;
        }
        $.serverPost('News.setFileDownload',{id:id_file,id_news:self.currentItem.id,url:url,position:$(li).index(),lang:lang},function (response_id){
    ...
    ,setupFiles : function (item){
        ...
        var setController = function (){
            ...
            var createRow = function (){
                ...
                $(li).find('select[name="lang"]').change(function (){
                    var id = $(li).attr('data-id-file');
                    if (id ) {
                        $.serverPost('News.fileDownloadChangeLang',{id:id,lang:($(this).val()||null)},function (){
                            self.showSaveMsg();
                        })
                    }
                });
            ...
            controller.insertFile= function (arg){
                ...
                var select_lang = $(li).find('select[name="lang"]');
                $('<option value="' + window.lang +'">' + window.langs[window.lang] + '</option>').appendTo(select_lang);
                if (arg['lang']!=undefined) {
                    select_lang.val(arg['lang']);
                }
            }
...
//open_file_dialog        
    ...
    ,setupWorkspace : function (){
        ...
        $('#btn-save,[data-role="save"]').click(function (event){
        ...
        $('#ctrl-contenuto').on('shown',function (event){
            ...
            if($.isFunction(window['contentEditorCreate'])) {
                //possibile definire function contentEditorCreate
                News.contentEditor = window['contentEditorCreate'].call(content_textarea);
            }
            else {
    ...
    ,fireEvent: function (eventname,params){
        var myEvent = jQuery.Event(eventname);
        ...
        if(myEvent.isDefaultPrevented()) return false; // check isDefaultPrevented
        return myEvent;
    }    
    ,getContent : function (){
        var c =News._getEditorValue.call(content_textarea);
        return c;
    }
    ,setContent : function (s){
        News._setEditorValue.call(content_textarea,s);
       
        News.fireEvent('News.setContent',{textarea:content_textarea});
    }
