...
private function AddUrlImage ($url) {
  if(stripos($url,'https:')===0) {
	//il protocollo https viene convertito in http
    ...
    if(isset($images[2])) {
      foreach($images[2] as $i => $url) {
        ...
        if (!preg_match('/^[A-z][A-z]*:\/\//',$url)) {
		  ...
		  if($this->EmbedRemoteImages) {
			  $params=$this->AddUrlImage($url);
			  if ( $this->AddEmbeddedImage($params['path'], md5($params['filename']), $params['filename'], 'base64',$params['mime']) ) {
            $message = preg_replace("/".$images[1][$i]."=\"".preg_quote($url, '/')."\"/Ui", $images[1][$i]."=\"".$params['cid']."\"", $message);
			}
			 /* $this->AddStringEmbedAttachment($params['raw'],$params['cid'],$params['filename'],'base64',$params['mime']);
			  $message = preg_replace("/".$images[1][$i]."=\"".preg_quote($url, '/')."\"/Ui", $images[1][$i]."=\"".$params['cid']."\"", $message);*/
		  }
		}
      }
