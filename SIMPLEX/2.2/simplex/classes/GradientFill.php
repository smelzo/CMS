 ...
 *
 * 1.
 * require_once('/home/ozh/www/includes/GradientFill.php');
 * $image = new gd_gradient_fill(200,200,'horizontal','#fff','#f00');
 *
 * 2.
 * require_once('c:/iis/inet/include/GradientFill.php');
...
class GradientFill {
	...
	function GradientFill($w,$h,$d,$s,$e,$step=0,$bDiplay=false) {
		$this->width = $w;
		$this->height = $h;
		$this->direction = $d;
		$this->startcolor = $s;
		...
		//echo $this->startcolor,$this->endcolor;
		//$this->display($this->image);die;
		// Show it		
		if($bDiplay)$this->display($this->image);
